...
test_type_checking(State) ->
    ...
    %% Test isintegral
    {ok, [true, true, false], State2} = luerl:do(<<"
        local n = bint.new(42)
    ...
    %% Test type function
    {ok, [<<"bint">>, <<"integer">>, <<"float">>], _State3} = luerl:do(<<"
        local n = bint.new(42)
...
test_addition(State) ->
    ...
    {ok, [<<"2000000000000000000">>], State2} = luerl:do(<<"
...
test_subtraction(State) ->
    ...
    {ok, [<<"500000000000000000">>], _State3} = luerl:do(<<"
...
test_multiplication(State) ->
    ...
    {ok, [<<"1000000000000000000000000000000000000">>], State2} = luerl:do(<<"
        local a = bint.new('1000000000000000000')
        local b = bint.new('1000000000000000000')
        local c = a * b
        return tostring(c)
...
test_very_large_numbers(State) ->
    ...
    %% Test power of 2
    {ok, [<<"1267650600228229401496703205376">>], _State3} = luerl:do(<<"
...
test_comparisons(State) ->
    ...
    {ok, LargeResults, State3} = luerl:do(<<"
...
test_power(State) ->
    ...
    %% Test large power
    {ok, [<<"1000000000000000000000000">>], _State3} = luerl:do(<<"
...
test_edge_cases(State) ->
    ...
    ">>, State) of
        {ok, _, _} -> no_error;
        {lua_error, {assert_error, <<"attempt to divide by zero">>}, _} -> caught_error;
        {lua_error, _, _} -> caught_error
    end,
    ?assertEqual(caught_error, DivZeroResult),
    
    %% Test with negative numbers
    {ok, NegResults, State1} = luerl:do(<<"
        local a = bint.new(-42)
    ...
    %% Test even/odd
    {ok, EvenOddResults, _State2} = luerl:do(<<"
